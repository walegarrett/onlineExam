        ...
        <div class="blog-main">
            <div class="blog-main">
                <div class="child-nav shadow">
                    <span class="child-nav-btn child-nav-btn-this" id="childBtn1"><a id="allPaper">全部考试</a></span>
                    <span class="child-nav-btn" id="childBtn2"><a id="notstart">未开始</a></span>
                    <span class="child-nav-btn" id="childBtn3"><a id="hasend">已结束</a></span>
                    <span class="child-nav-btn" id="childBtn4"><a id="during">正在进行</a></span>
                </div>
...
<script>
    function buildPaperPage(result){
        ...
        $.each(paperlist,function (index,item) {//所有的版块
            ...
            var hourandminute=formatDateHourAndMinute(item.startTime);
            var abstract=$("<p class='resource-abstract'></p>").append("开始考试时间: "+date+" "+hourandminute+" 时长: "+item.durationTime+"分");
            ...
            //考试是否开始或者结束
            if(item.hasDoExam)
                var sectionC=$("<a class='layui-btn layui-btn-small layui-btn-primary'>已完成</a>")
                    .append($("<i class='fa fa-eye fa-fw'></i>"));
            else if(now_time<start_time)
                var sectionC=$("<a class='layui-btn layui-btn-small layui-btn-primary'>考试未开始</a>")
                    .append($("<i class='fa fa-eye fa-fw'></i>"));
            else if(now_time>end_time)
                var sectionC=$("<a class='layui-btn layui-btn-small layui-btn-primary'>考试已结束</a>")
                    .append($("<i class='fa fa-eye fa-fw'></i>"));
            else var sectionC=$("<button class='layui-btn layui-btn-small layui-btn-primary startExam' id='"+item.id+"'>开始考试</button>")
                    .append($("<i class='fa fa-eye fa-fw'></i>"));
            sectionC.attr("inviCode",item.inviCode);
            var sectionD=$("<a class='layui-btn layui-btn-small layui-btn-primary' target='_blank'>详情</a>")
    ...
    $(function () {
        $("#papercenterli").addClass("layui-this");
        $("#papercenterli1").addClass("layui-this");
       to_page(1);
        showNoReadCount("${APP_PATH}","${userid}");
    });
    function to_page(pn){
        if($("#childBtn1").hasClass("child-nav-btn-this")){
            buildPageAllPaper(pn);
        }else if($("#childBtn2").hasClass("child-nav-btn-this")){
            buildNotStartPage(pn);
        }else if($("#childBtn3").hasClass("child-nav-btn-this")){
            buildHasEndPage(pn);
        }else if($("#childBtn4").hasClass("child-nav-btn-this")){
            buildDuringPage(pn);
        }
    }
    function buildPageAllPaper(pn){
        $.ajax({
            data:{"pn":pn},
            url:"${APP_PATH}/findAllPaper",
            type:"GET",
    ...
    function buildHasEndPage(pn){
        $.ajax({
            data:{"pn":pn},
            url:"${APP_PATH}/findHasEndPaper",
            type:"GET",
            success:function(result){
                buildPaperPage(result);
                //2.解析并且显示分页信息
    ...
    $("#hasend").click(function () {
        // alert("已结束");
        buildHasEndPage(1);
    });
    function buildDuringPage(pn){
        $.ajax({
            data:{"pn":pn},
            url:"${APP_PATH}/findDuringPaper",
            type:"GET",
            success:function(result){
                buildPaperPage(result);
                //2.解析并且显示分页信息
    ...
    function buildNotStartPage(pn){
        $.ajax({
            data:{"pn":pn},
            url:"${APP_PATH}/findNotStartPaper",
            type:"GET",
            success:function(result){
                buildPaperPage(result);
                //2.解析并且显示分页信息
